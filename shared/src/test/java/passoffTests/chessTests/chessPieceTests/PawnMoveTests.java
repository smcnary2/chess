...
public class PawnMoveTests {
    ...
    private void validatePromotion(String boardText, chess.ChessPosition startPosition, int[][] endPositions) {
        var board = TestFactory.loadBoard(boardText);
        var testPiece = board.getPiece(startPosition);
        var validMoves = new HashSet<chess.ChessMove>();
        for (var endPosition : endPositions) {
            var end = startPosition(endPosition[0], endPosition[1]);
            validMoves.add(TestFactory.getNewMove(startPosition, end, chess.ChessPiece.PieceType.QUEEN));
            validMoves.add(TestFactory.getNewMove(startPosition, end, chess.ChessPiece.PieceType.BISHOP));
            validMoves.add(TestFactory.getNewMove(startPosition, end, chess.ChessPiece.PieceType.ROOK));
            validMoves.add(TestFactory.getNewMove(startPosition, end, chess.ChessPiece.PieceType.KNIGHT));
        }
